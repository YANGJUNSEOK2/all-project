...
bool Application::CanStartUp()
{
	...
	window = SDL_CreateWindow("APEX", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
...
void Application::Update()
{
    ...
    // remove this since we will include this in pause state
    /*
    ...
    // if toggle fullscreen is selected in pause state, toggle fullscreen
    if(game.GetCurrentState()->GetId() == LV_Pause && 
        (m_input.IsTriggered(SDL_SCANCODE_RETURN) || m_input.IsTriggered(SDL_SCANCODE_SPACE)))
    {
        if(game.pauseLevel.pauseImageStatus == PauseState::TOGGLE_FULLSCREEN)
        {
	...
	if (frame_time >= 1.0)
	{
		...
		frame_time = 0;
		std::stringstream ss;
		ss << "APEX [fps=" << static_cast<int>(fps) << "]";
