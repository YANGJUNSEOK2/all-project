...
template<typename T> struct Matrix3 {
    ...
    Matrix3(Vector3<T> Zero_column, Vector3<T> first_column, Vector3<T> second_column) noexcept
    {
    ...
    Vector3<T> GetColumn(T p_Index) noexcept {
        p_Index *= 3;
    ...
    //() Operator Override
    template<typename T>
    T operator()(T column, T row) const noexcept
    {
    ...
    //() Operator Override
    template<typename T>
    T& operator()(T column, T row) noexcept
    {
        assert((column >= 0) && (column < 3));
        assert((row >= 0) && (row < 3));
        return Elements[column][row];
...
template<typename T>
Matrix3<T> build_rotation(T angle_in_radians) noexcept
{
...
template<typename T>
Matrix3<T> build_scale(T scale_x, T scale_y) noexcept
{
    return Matrix3<T>(scale_x, 0, 0, 0, scale_y, 0, 0, 0, 1);
}

template<typename T>
Matrix3<T> build_scale(T scale) noexcept
{
...
template<typename T>
Matrix3<T> build_translation(T translate_x, T translate_y) noexcept
{
