...
void RoomExit_Print(RoomExit* roomExitList)
{
	...
	unsigned int exitPrintCounter; /* used to track the number of exits that have been shown to the user */
	...
	roomExitIndex = 0;
	exitPrintCounter = 1;	/* 1 indexed counter */
	...
	while (roomExitCurrent != NULL)
	{
		...
		if (roomExitIndex > 0)
		{
			...
			if (exitPrintCounter == numRoomExits || exitPrintCounter==0)
				printf(" ");
			else if(exitPrintCounter != numRoomExits)
