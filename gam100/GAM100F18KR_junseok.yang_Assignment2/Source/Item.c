...
Item* Item_Create(const char* name, const char* description, bool isCarryable,bool isDrinkable, ItemFunc useFunc, ItemFunc takeFunc, ItemFunc dropFunc)
{
...
bool Item_IsDrinkable(Item* item)
{
	/* return the data if the parameter is not NULL, otherwise return false (not carryable) */
	return (item != NULL) ? item->isDrinkable : false;
}
